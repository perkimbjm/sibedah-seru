...
<div class="card">
    <div class="card-header">
        Edit Data Bedah Rumah
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("app.houses.update", [$house->id]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            
            <div class="form-group">
                <label class="required" for="nik">NIK</label>
                <input class="form-control {{ $errors->has('nik') ? 'is-invalid' : '' }}" type="number" name="nik" id="nik" value="{{ old('nik', $house->nik) }}" required pattern="[0-9]{16}" placeholder="Isi Nomor Identitas yang Valid (16 digit)">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $house->name) }}" required placeholder="Masukkan nama Penerima Bantuan">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $house->address) }}" required placeholder="Masukkan Alamat (dengan nama desa)">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('village_id') ? 'is-invalid' : '' }}" name="village_id" id="village_id" required>
                    @foreach($villages as $id => $village)
                        ...
                        <option value="{{ $id }}" {{ old('village_id', $house->village_id) == $id? 'selected' : '' }}>{{ $village }}</option>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="text" name="lat" id="lat" value="{{ old('lat', $house->lat) }}" onfocus="this.value='-2.';">
                        @if($errors->has('lat'))
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="text" name="lng" id="lng" value="{{ old('lng', $house->lng) }}" onfocus="this.value='115.';">
                        @if($errors->has('lng'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number" name="year" id="year" value="{{ old('year', $house->year) }}" required min="2020" max="2029" oninput="this.value = this.value.slice(0, 4);">
                @if($errors->has('year'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $house->type) }}" required placeholder="Jenis Bantuan / Kegiatan">
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('source') ? 'is-invalid' : '' }}" type="text" name="source" id="source" value="{{ old('source', $house->source) }}" required placeholder="Contoh : APBD / APBN / APBD Prov / CSR/ Lainnya">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note" placeholder="Catatan atau keterangan tambahan, jika ada">{{ old('note', $house->note) }}</textarea>
                @if($errors->has('note'))
...
<x-coordinate :lat="$house->lat" :lng="$house->lng" />
@endsection
