...
<div class="map-container">
    ...
    <div id="sidebar" class="active">
        @include('rtlh.partials.sidebar')
    </div>

    <div id="sidebar-edit" class="active">
        @include('rtlh.partials.sidebar-edit')
    </div>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="my-2 ml-auto mr-2">
                ...
                <a href="{{ route('app.rtlh.index') }}" class="my-2 btn btn-success" style="color: white !important;">Kembali ke Tabel</a>
            </div>
        </nav>
...
<script>
    ...
    const createPopupContent = (properties) => {
        const { name, id, address, status_safety, status } = properties;
        return `
            ${name}
            <br>ID: ${id}
            <br>Alamat: ${address}
            <br>Kelayakan Bangunan: ${status_safety}
            <br>Kelayakan Bangunan dan Sanitasi: ${status}
            <br><button class="mt-2 btn btn-sm bg-danger" onclick="RtlhEditor.editRtlh(${id})">Edit</button>
        `;
    };
    ...
    const loadMapData = async () => {
        try {
            const response = await fetch('{{ route('api.rtlh') }}');
            if (!response.ok) {
    ...
    const FormHandler = {
        init: () => {
            document.getElementById('rtlhData').addEventListener('submit', FormHandler.handleSubmit);
            document.getElementById('rtlhEditData').addEventListener('submit', FormHandler.handleEditSubmit);
        },
    ...
    const UIHelper = {
        ...
        resetFormAndCloseSidebar: () => {
            document.getElementById('rtlhData').reset();
        ...
        resetEditFormAndCloseSidebar: () => {
            document.getElementById('rtlhEditData').reset();
            $("#sidebar-edit").addClass("active");
            if (LocationPicker.isActive) {
                LocationPicker.deactivate();
            }
    ...
    const RtlhEditor = {
        editRtlh: (id) => {
            ...
            fetch(`{{ route("app.rumah.getData") }}?id=${id}`)
                .then(response => {
        ...
        fillEditForm: (data) => {
            const fields = ['id', 'nik', 'kk', 'name', 'address', 'village_id', 'district_id', 'people', 'area', 'lat', 'lng', 'pondasi', 'kolom_blk', 'rngk_atap', 'atap', 'dinding', 'lantai', 'air', 'jarak_tinja', 'wc', 'jenis_wc', 'tpa_tinja', 'status_safety', 'status', 'is_renov', 'note'];
